...
contract CPTStaking is Staking, IMinter {
    ...
    constructor(address cusd, address cpt){
    ...
    function notifyMint(uint256 cusdAmount, uint256 cptAmount) public override onlyPools {

        if (cptAmount > 0) {

            coinCPT.burn(cptAmount);

            uint256 leftRatio = (_totalStaking - cptAmount) * SHARE_PRECISION / _totalStaking;
            uint256 newUnusedRatio = shareUnusedRatio * leftRatio / SHARE_PRECISION;
            if (newUnusedRatio > 0)
                shareUnusedRatio = newUnusedRatio;

            _totalStaking -= cptAmount;
        }
    ...
    function stake(uint256 amount) external override nonReentrant whenNotPaused updateReward(msg.sender) updateMint(msg.sender) {
        ...
        SafeERC20.safeTransferFrom(coinCPT, msg.sender, address(this), amount);
    ...
    function stakeWithReferral(uint256 amount, address referral) external nonReentrant whenNotPaused updateReward(msg.sender) updateMint(msg.sender) {
        ...
        uint256 stakeAmount = _transferWithReferral(coinCPT, amount, referral);
    ...
    function notifyReinvest(address account, uint256 amount) external override updateMint(account) updateReward(account) onlyStakings {
        require(_totalStaking + amount <= coinCPT.balanceOf(address(this)) - rewardBalance, "Notify amount exceeds balance!");
    ...
    function withdraw(uint256 shareAmount) public override nonReentrant updateReward(msg.sender) updateMint(msg.sender) {
        ...
        SafeERC20.safeTransfer(coinCPT, msg.sender, withdrawAmount);
    ...
    function yieldAPR() public override view returns (uint256){
        if (_totalStaking > 0)
            return rewardRate * 31536000 * CAPC_DECIMAL / _totalStaking;
        else
            return 0;
    }


    function getRewardWithPercent(uint256 percent) public override nonReentrant updateReward(msg.sender) {
        require(percent > 0 && percent <= 100, "percent wrong");
        uint256 reward = rewards[msg.sender] * percent / 100;
        if (reward > 0) {
            rewards[msg.sender] -= reward;
    ...
    function notifyRewardAmount(uint256 reward) external override onlyOwner updateReward(address(0)) {
        ...
        uint balance = coinCPT.balanceOf(address(this)) - _totalStaking;
        require(rewardRate <= balance.div(rewardsDuration), "Provided reward too high");
